...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // TEMPORARY: Auto-bypass authentication
    console.log('🔓 Authentication bypassed for development')
    bypassAuth()
    return

    // Original auth code (commented out for bypass)
    /*
  ...
  const withTimeout = <T,>(promise: Promise<T>, timeoutMs: number = 15000): Promise<T> => {
    return Promise.race([
      ...
      new Promise<T>((_, reject) =>
        setTimeout(() => reject(new Error('Request timeout. Please check your internet connection and try again.')), timeoutMs)
      )
    ])
  }
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      if (data.user) {
        ...
        const userProfile = await withTimeout(fetchUserProfile(email), 10000)
      ...
      return { success: false, error: error.message || 'Login timeout. Please check your internet connection and try again.' }
    } finally {
      setLoading(false)
    }
  }
