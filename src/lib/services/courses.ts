...
export class CourseService {
   ...
   * Update course
   */
  static async updateCourse(id: string, updates: UpdateCourseData): Promise<Course> {
    try {
      ...
      console.log('Course updated successfully:', data.name)
      return data
    } catch (error: any) {
  ...
  static async deleteCourse(id: string): Promise<void> {
    try {
      // Check if course has active assessments
      ...
      if (assessments && assessments.length > 0) {
        throw new Error('Cannot delete course with existing assessments')
      }
