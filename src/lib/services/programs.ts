...
export class ProgramService {
   ...
   * Update program
   */
  static async updateProgram(id: string, updates: UpdateProgramData): Promise<Program> {
    try {
      ...
      console.log('Program updated successfully:', data.name)
      return data
    } catch (error: any) {
  ...
  static async deleteProgram(id: string): Promise<void> {
    try {
      ...
      if (courses && courses.length > 0) {
        throw new Error('Cannot delete program with active courses')
      }
